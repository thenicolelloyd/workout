...
# Find the exact block to replace - from focusKey through eqList
old = """  const focusKey=S.muscles.includes('full_body')?'full_body':S.muscles.join('_');
...
if old in content:
    ...
    print('✓ 1. Recency scoring + circuit timing + legs note injected')
else:
    print('✗ 1. Could not find target block')

# Update the user prompt to use all the new variables
...
if old_prompt in content:
    ...
    print('✓ 2. User prompt updated with all variables')
else:
    print('✗ 2. Could not find user prompt — may already be updated')
